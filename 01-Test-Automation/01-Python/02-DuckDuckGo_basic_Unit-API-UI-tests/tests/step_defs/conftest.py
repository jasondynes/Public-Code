import pytest

from pytest_bdd import given
from selenium import webdriver

# Constants

DUCKDUCKGO_HOME = 'https://duckduckgo.com/'
RAILSBANK_API = 'https://playlive.railsbank.com/v1/customer/beneficiaries'

#file cleanup
import os
if os.path.exists("api_response.json"):
  os.remove("api_response.json")

# Hooks

def pytest_bdd_step_error(request, feature, scenario, step, step_func, step_func_args, exception):
    print(f'Step failed: {step}')


# Fixtures

@pytest.fixture
def browser():
    # For this example, we will use Firefox
    # You can change this fixture to use other browsers, too.
    # A better practice would be to get browser choice from a config file.
    #driver = webdriver.Chrome('C:/Users/jason/.virtualenvs/TestAutomationUniversity_PythonBDD-LjdB6Afi/chromedriver.exe')
    b = webdriver.Chrome()
    b.implicitly_wait(20)
    yield b
    b.quit()

# Shared Given Steps

@given('the DuckDuckGo home page is displayed')
def ddg_home(browser):
    browser.get(DUCKDUCKGO_HOME)
