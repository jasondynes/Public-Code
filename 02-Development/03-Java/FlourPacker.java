...
public class FlourPacker {
    ...
    public static boolean canPack(int bigCount, int smallCount, int goal){
        ...
        if ((bigCount * 5) + smallCount >= goal){
            // do we have enough small bags?
            int smallReqd = goal % 5;
            if (smallReqd > smallCount){
                return false;
            }
            // take small bags away to then check large ones needed
            int bigReqd = (goal - smallReqd) / 5;
            if (bigReqd <= bigCount || (goal - (bigReqd * 5) - smallReqd <= 0)){
                return true;
            }
        }
        return false;
